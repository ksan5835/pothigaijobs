						...
						<?php foreach ( $job_dashboard_columns as $key => $column ) : ?>
							<td class="<?php echo esc_attr( $key ); ?>">
								...
								<?php elseif ('applications' === $key ) : ?>
									<?php 
										global $post;
										echo ( $count = get_job_application_count( $job->ID ) ) ? '<a class="button" href="' . add_query_arg( array( 'action' => 'show_applications', 'job_id' => $job->ID ), get_permalink( $post->ID ) ) . '">Show (' . $count . ')</a>' : '&ndash;';
 									?>
								<?php else : ?>
